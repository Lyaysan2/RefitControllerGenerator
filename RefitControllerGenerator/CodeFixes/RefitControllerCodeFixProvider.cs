...
namespace RefitControllerGenerator.CodeFixes
{
    ...
    public sealed class RefitControllerCodeFixProvider : CodeFixProvider
    {
        ...
        private static ClassDeclarationSyntax GenerateControllerClass(string controllerName, string interfaceName, INamedTypeSymbol interfaceSymbol)
        {
            ...
            var docs = GenerateControllerDocs(interfaceSymbol);
            if (docs != null)
                classDecl = classDecl.WithLeadingTrivia(
                    SyntaxFactory.TriviaList(
                        SyntaxFactory.Trivia(docs),
                        SyntaxFactory.ElasticCarriageReturnLineFeed));
        ...
        private static MethodDeclarationSyntax GenerateControllerMethod(IMethodSymbol method)
        {
            ...
            var docs = GenerateMethodDocs(method);
            if (docs != null)
            {
                methodDecl = methodDecl.WithLeadingTrivia(
                    SyntaxFactory.TriviaList(
                        SyntaxFactory.Trivia(docs),
                        SyntaxFactory.ElasticCarriageReturnLineFeed));
            }
        ...
        /// <param name="interfaceSymbol"></param>
        /// <returns></returns>
        private static DocumentationCommentTriviaSyntax GenerateControllerDocs(INamedTypeSymbol interfaceSymbol)
        {
            ...
            var formatted = string.Join(nl, lines);

            var trivia = SyntaxFactory.ParseLeadingTrivia(
                "/// " + formatted.Replace(nl, nl + "/// "))
                .FirstOrDefault();
        ...
        /// <returns></returns>
        private static DocumentationCommentTriviaSyntax? GenerateMethodDocs(ISymbol symbol)
        {
            ...
            return SyntaxFactory.DocumentationCommentTrivia(
                ...
                SyntaxFactory.ParseSyntaxTree("/// " +
                    content.Replace(Environment.NewLine, Environment.NewLine + "/// "))
                .GetRoot()
                .ChildNodes()
        ...
        private static string GetServiceFieldName(string interfaceName)
        {
            ...
            return char.ToLowerInvariant(interfaceName[1]) +
                   interfaceName.Substring(2) + "Service";
        }
        ...
        private static string GetControllerName(string interfaceName)
            => interfaceName.StartsWith("I", StringComparison.Ordinal)
                ? interfaceName.Substring(1) + "Controller"
